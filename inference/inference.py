...
SAMPLE_RATE = 16000
SLICE_LEN = 65536  # your checkpoint is for 65536
...
def generate_final(z: np.ndarray, ckpt_path: str) -> np.ndarray:
    """Return final mono waveform (np.float32) from one forward."""
...
def generate_evolution_pairs(z: np.ndarray, ckpt_path: str, output_dir: str):
    """
    One forward pass with hooks; for each layer:
      - write RAW:   <name>_raw.wav  (native length)
      - write STRETCHED: <name>_stretched.wav (resampled to final length)
    Returns: list of dicts [{name, raw_path, stretched_path, raw_len, stretched_len}], in order.
    """
    ...
    # register hooks on the actual modules used in forward()
    hs = []
    try:
        ...
        final_t = G(z_t).detach().cpu()  # (1,1,65536)
    finally:
        for h in hs:
            h.remove()
    ...
    # Append the true final output as "final"
    final_raw = final  # already 65536
    final_raw_path = os.path.join(output_dir, "final_raw.wav")
    final_stretched_path = os.path.join(output_dir, "final_stretched.wav")  # same as raw
    _write_wav(final_raw, final_raw_path, normalize=True)
    _write_wav(final_raw, final_stretched_path, normalize=True)
